// Pre-upload video trimmer. Plays the clip (expo-av, works on web + native) and
// lets the user mark a start and end before saving. The actual cut runs in
// ffmpeg.wasm on the web (see src/platform/trim.web.ts); the parent only shows
// this when trimSupported is true, so it never runs where there's no transcoder.
...
function fmt(ms: number): string {
  ...
  const s = total % 60;
  const cs = Math.floor((ms % 1000) / 100); // tenths
...
export function TrimModal({
  ...
  function onStatus(s: AVPlaybackStatus) {
    ...
    if (s.durationMillis && s.durationMillis !== dur) {
      setDur(s.durationMillis);
      if (end === 0) setEnd(s.durationMillis); // first load: select whole clip
    }
    setPos(s.positionMillis ?? 0);
    // Auto-pause at the end marker so previewing the selection feels bounded.
    if (end > 0 && (s.positionMillis ?? 0) >= end && s.isPlaying) {
      ref.current?.pauseAsync().catch(() => {});
    }
  }

  // Keep markers sane if the user inverts them.
  useEffect(() => {
    if (end > 0 && start > end) setStart(end);
  }, [end, start]);

  const seg = Math.max(0, end - start);
  const canApply = dur > 0 && seg >= 200 && !working; // need ≥0.2s

  async function preview() {
    await ref.current?.setPositionAsync(start).catch(() => {});
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onCancel}>
      <View style={{ flex: 1, backgroundColor: "#000" }}>
        ...
        {/* video — fill the available space (expo-av's web <video> won't stretch
            on its own in a flex column, so pin it to fill an explicit box) */}
        ...
        {/* range bar: full clip with the selected segment highlighted + playhead */}
        <View style={{ paddingHorizontal: 18, paddingTop: 14 }}>
          <View style={{ height: 8, borderRadius: 4, backgroundColor: "rgba(255,255,255,0.18)", justifyContent: "center" }}>
            <View style={{ position: "absolute", left: pct(start), right: undefined, width: pct(seg), height: 8, borderRadius: 4, backgroundColor: theme.accent2 }} />
